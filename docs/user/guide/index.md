...
Start the Web UI through the [root README](../../../README.md#run); the command prints its URL. `pnpm run desktop` opens the same UI in a native window ([desktop shell](../../../apps/electron/README.md)). This guide begins after that server is running. The `dsh` process uses its invoking directory as the default filesystem location, but a fresh Web UI has no selected workspace until you add one.
