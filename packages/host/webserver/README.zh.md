...
该包不了解任何 harness 概念，也不提供任何文件服务：`/api` HTTP 桥接与下行 WebSocket 是 connection 插件的 route，插件 bundle 与 HMR（热模块替换）事件流是 modules／hmr 插件的 route，dist 服务则属于 fallback 持有者。upgrade handler 拥有协议握手与连接内容；webserver 只交付原始 socket 与 request。`host` 只接受 `127.0.0.1`（默认安全姿态）和 `0.0.0.0`（有意向网络开放）。浏览器与位于 [`apps/electron`](../../../apps/electron/README.md) 的桌面 shell 使用本服务器；桌面应用把 loopback URL 加载到 Electron 窗口中。`file://` 加 IPC 的载体尚未交付。该包从不打印内容；URL 行属于 shell。
