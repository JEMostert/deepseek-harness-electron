...
The package knows no harness concepts and serves no files: the `/api` HTTP bridge and downlink WebSockets are routes owned by the connection plugin, plugin bundles and the HMR event stream are routes owned by the modules/hmr plugins, and dist serving belongs to the fallback owner. The upgrade handler owns the protocol handshake and connection contents; the webserver only delivers the raw socket and request. `host` accepts only `127.0.0.1` (default posture) and `0.0.0.0` (deliberate network exposure). Browsers and the desktop shell at [`apps/electron`](../../../apps/electron/README.md) consume this server; the desktop app loads the loopback URL in an Electron window. A `file://` plus IPC carrier is not shipped. This package never prints; the URL line belongs to the shell.
